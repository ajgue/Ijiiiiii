...
function MakeWindow(Configs)
  ...
  function AddToggle(parent, Configs)
    ...
    local TextButton = Create("TextButton", parent, {
        ...
        Text = "",
        AutoButtonColor = false
    }) Corner(TextButton, {CornerRadius = UDim.new(1, 0)}) Stroke(TextButton)
    ...
    if Default then
        OnOff = true
        Frame2.Position = UDim2.new(0, 19, 0.5, 0)
    ...
    TextButton.MouseButton1Click:Connect(function()
        Sound:Play()
        if Frame2.Position.X.Offset < 10 then
            OnOff = true
            CreateTween(Frame2, "Position", UDim2.new(0, 19, 0.5, 0), 0.2, false)
            ...
            OnOff = false
            CreateTween(Frame2, "Position", UDim2.new(0, 1, 0.5, 0), 0.2, false)
  ...
  function AddColorDropdown(parent, targetFrame, Configs)
    local name = Configs.Name or "Color Theme"
    local Default = Configs.Default or Color3.fromRGB(0, 0, 200)
    local Callback = Configs.Callback or function() end
    local Colors = {
        ...
        {Name = "بنفسجي", Color = Color3.fromRGB(255, 0, 255)},
        {Name = "سماوي", Color = Color3.fromRGB(0, 255, 255)},
        {Name = "أبيض", Color = Color3.fromRGB(255, 255, 255)}
    }

    -- الحاوية الرئيسية
    local main = Create("Frame", parent, {
        Size = UDim2.new(1, 0, 0, 30),
        BackgroundColor3 = Configs_HUB.Cor_Options,
        ClipsDescendants = true
    }) Corner(main) Stroke(main)

    -- زر التبديل
    local button = Create("TextButton", main, {
        Size = UDim2.new(1, 0, 0, 30),
        BackgroundTransparency = 1,
        Text = name,
        TextColor3 = Configs_HUB.Cor_Text,
    ...
    -- قائمة الألوان
    local dropdown = Create("Frame", main, {
        Position = UDim2.new(0, 0, 1, 0),
        Size = UDim2.new(1, 0, 0, 0),
        BackgroundTransparency = 1,
        ClipsDescendants = true
    ...
    button.MouseButton1Click:Connect(function()
        ...
        main:TweenSize(
            UDim2.new(1, 0, 0, open and (30 + targetSize) or 30),
            Enum.EasingDirection.Out,
            Enum.EasingStyle.Sine,
            0.25,
            true
    ...
    for _, colorInfo in ipairs(Colors) do
        local colorBtn = Create("TextButton", dropdown, {
            Size = UDim2.new(1, -10, 0, 25),
            BackgroundColor3 = colorInfo.Color,
            Text = colorInfo.Name,
            Font = Configs_HUB.Text_Font,
            TextColor3 = Color3.fromRGB(0, 0, 0),
            TextSize = 11
        }) Corner(colorBtn) Stroke(colorBtn)

        colorBtn.MouseButton1Click:Connect(function()
            if targetFrame and targetFrame:IsA("GuiObject") then
                targetFrame.BackgroundColor3 = colorInfo.Color
            end
            Callback(colorInfo.Color)
        end)
    end
end
